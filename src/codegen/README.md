...
10. RISC-V64 常量有符号除法/余数保持 `divw/remw` 的向零截断、负除数、`INT_MIN/-1` 和除零语义：除以 2 时直接从 sign bit 形成 bias，其它二次幂保持完整 bias，余数的 `quotient * 2^k` 用 `slliw`，输入直接从已分配源寄存器参与 magic/shift 序列，结果也直接写入已分配目标寄存器。`MemZero` 不假设额外指针对齐，而是每次用八个 byte store 清零固定八字节 chunk，再处理 0..7 字节静态 tail，把逐字节分支循环的控制开销摊薄八倍。
