...
#[cfg(test)]
mod tests {
    use super::super::{ModulePass, ScalarPromotePass};
    use super::*;
    use crate::ir::lower::lower_program;
    use crate::parser::Parser;

    fn lower_and_promote(source: &str) -> Module {
    ...
    #[test]
    fn wraps_pure_branching_recursion_without_using_its_name() {
        let mut module = lower_and_promote(
            r#"
            int __recursive_memo_tree_0_tags;
            int tree(int a, int b) {
                if (a == 0 || b == 0) return 0;
                return tree(a - 1, b) + tree(a - 1, b - 1);
            }
            int main() { return tree(10, 10); }
            "#,
        );
        RecursiveMemoizePass::new().run(&mut module);
        assert!(module.funcs.iter().any(|func| func.name == "tree"));
        assert!(module
            .funcs
            .iter()
            .any(|func| func.name.starts_with("__recursive_memo_tree_1_")));
        assert!(module
            .globals
            .iter()
            .any(|global| global.name.ends_with("_tags")));
        assert!(module.funcs.iter().all(|func| func.verify().is_ok()));
    }

    #[test]
    fn rejects_recursion_with_observable_writes() {
        let mut module = lower_and_promote(
            r#"
            int count;
            int tree(int a, int b) {
        ...
        assert!(!module
            .funcs
            .iter()
            .any(|func| func.name.starts_with("__recursive_memo_tree_")));
        assert!(module.funcs.iter().all(|func| func.verify().is_ok()));
    }
}
