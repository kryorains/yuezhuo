...
/// Marks the canonical recursive "double-and-add modulo" idiom for a guarded
/// RV64 lowering.  The marker does not change IR semantics: code generation
/// emits the original body as a fallback for values outside the proven range.
pub(super) struct GuardedMulModPass;

impl GuardedMulModPass {
    pub(super) fn new() -> Self {
        Self
    }
}

impl ModulePass for GuardedMulModPass {
    fn run(&mut self, module: &mut Module) {
        for func in &mut module.funcs {
            if let Some(modulus) = match_recursive_mulmod(func) {
...
fn match_recursive_mulmod(func: &Function) -> Option<i32> {
    ...
    // This bound proves both cur + cur and cur + lhs stay in signed i32.
...
mod tests {
    ...
    fn optimized(source: &str) -> crate::ir::Module {
        ...
        run_pipeline(
            ...
            PassOptions {
                ...
                enable_guarded_mulmod_idiom: true,
    ...
    #[test]
    fn marks_exact_recursive_double_and_add_modulo() {
        let module = optimized(
            ...
            int mulmod(int a, int b) {
                if (b == 0) return 0;
                if (b == 1) return a % mod;
                int cur = mulmod(a, b / 2);
                cur = (cur + cur) % mod;
                if (b % 2 == 1) return (cur + a) % mod;
                return cur;
            }
            int main() { return mulmod(3, 4); }
            "#,
        );
        let func = module
            .funcs
            .iter()
            .find(|func| func.name == "mulmod")
            .unwrap();
        assert_eq!(func.guarded_mulmod_modulus(), Some(998244353));
        assert!(func.verify().is_ok());
    }

    #[test]
    fn rejects_a_near_match_with_a_changed_recurrence() {
        let module = optimized(
            r#"
            const int mod = 998244353;
            int almost(int a, int b) {
                if (b == 0) return 0;
                if (b == 1) return a % mod;
                int cur = almost(a, b / 2);
                cur = (cur + cur + 1) % mod;
                if (b % 2 == 1) return (cur + a) % mod;
                return cur;
            }
            int main() { return almost(3, 4); }
            "#,
        );
        let func = module
            .funcs
            .iter()
            .find(|func| func.name == "almost")
            .unwrap();
        assert_eq!(func.guarded_mulmod_modulus(), None);
    }
}
