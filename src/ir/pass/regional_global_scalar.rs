...
mod tests {
    ...
    #[test]
    fn hoists_repeated_readonly_global_loads_from_a_disjoint_loop() {
        let mut module = Module::new();
        for name in ["limit", "sink"] {
            module.globals.push(Global {
                name: name.into(),
                ty: Type::I32,
                is_const: false,
        ...
        let mut func = Function::new("read_limit", Type::I32);
        let keep_going = func.add_param("keep_going", Type::I1);
        let limit = func.add_global_ref("limit", Type::Ptr(Box::new(Type::I32)));
        let sink = func.add_global_ref("sink", Type::Ptr(Box::new(Type::I32)));
        let zero = func.add_const(Const::Int(0));
        ...
        let first = func
            .append_inst(body, InstKind::Load { ptr: limit }, Some(Type::I32))
            .unwrap();
        let second = func
            .append_inst(body, InstKind::Load { ptr: limit }, Some(Type::I32))
            .unwrap();
        ...
        RegionalInvariantGlobalLoadPass::new().run(&mut module);

        let func = &module.funcs[0];
        assert!(func.verify().is_ok());
        assert_eq!(
            func.blocks[func.entry.0]
                .insts
                .iter()
                .filter(|inst| matches!(inst.kind, InstKind::Load { ptr } if ptr == limit))
                .count(),
            1
        );
        assert!(func.blocks[body.0]
            .insts
            .iter()
            .all(|inst| !matches!(inst.kind, InstKind::Load { ptr } if ptr == limit)));
    }
}
