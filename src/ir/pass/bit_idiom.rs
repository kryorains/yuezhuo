...
fn install_fast_path(func: &mut Function, candidate: Candidate) {
    ...
    // When the loop result is returned immediately, the native version can
    // return on its own edge. This avoids introducing a merge phi that would
    // otherwise increase register pressure in the untouched fallback loop.
    let exit_returns_accumulator = func.blocks[candidate.exit.0]
        .insts
        .iter()
        .all(|inst| matches!(inst.kind, InstKind::Nop))
        && matches!(
            func.blocks[candidate.exit.0].terminator,
            Some(Terminator::Return(Some(value))) if value == candidate.accumulator
        );
    if exit_returns_accumulator {
        func.set_terminator(fast, Terminator::Return(Some(fast_result)));
        return;
    }
